...
setwd("/Users/Kyle/Dropbox/TI-seq/NextSeq/Renorm/")
...
#normalize each bin by total reads in bin
bin1 <- sum(All_reads$Reads_B1)
bin2 <- sum(All_reads$Reads_B2)
bin3 <- sum(All_reads$Reads_B3)
bin4 <- sum(All_reads$Reads_B4)
bin5 <- sum(All_reads$Reads_B5)

All_reads$Norm_B1 <- All_reads$Reads_B1/bin1
All_reads$Norm_B2 <- All_reads$Reads_B2/bin2
All_reads$Norm_B3 <- All_reads$Reads_B3/bin3
All_reads$Norm_B4 <- All_reads$Reads_B4/bin4
All_reads$Norm_B5 <- All_reads$Reads_B5/bin5


#get sum of all bins normalized
All_reads$total_bins <- All_reads$Norm_B1 + All_reads$Norm_B2 + All_reads$Norm_B3 + All_reads$Norm_B4 + All_reads$Norm_B5
All_reads$total_bins_unnorm <- All_reads$Reads_B1 + All_reads$Reads_B2 + All_reads$Reads_B3 + All_reads$Reads_B4 + All_reads$Reads_B5

#write counts for exp1
write.table(All_reads, "all_reads_e1.txt")

#sequences with less than 10 total counts
All_reads <- subset(All_reads, !total_bins_unnorm < 10)

#get summary statistics
sum(All_reads$total_bins_unnorm)
mean(All_reads$total_bins_unnorm)

#plot histogram of total counts
ggplot(All_reads, aes(log10(total_bins_unnorm))) + 
  ...
  annotate("text", x=3.5, y=15000, label=paste("Mean =", round(mean(All_reads$total_bins_unnorm),2))) +
  theme_science()
ggsave("hist_total_counts.tiff", height =4, width=4, units="in")

#get ratios of each bin to total
All_reads$B1_ratio <- All_reads$Norm_B1/All_reads$total_bins
All_reads$B2_ratio <- All_reads$Norm_B2/All_reads$total_bins
All_reads$B3_ratio <- All_reads$Norm_B3/All_reads$total_bins
All_reads$B4_ratio <- All_reads$Norm_B4/All_reads$total_bins
All_reads$B5_ratio <- All_reads$Norm_B5/All_reads$total_bins
...
rare_codons <- read.table("/Users/Kyle/Dropbox/TI-seq/NextSeq/Rare_codon_combinations.txt")
...
RNAfold <- read.delim("temp.score_rnafold.tsv")
RNAfold <- data.frame(Sequence = RNAfold$Sequence, RNAfold = RNAfold$rnafold.clean)

All_reads_annotated <- merge(All_reads, RNAfold, by = "Sequence", all.x = TRUE)

ggplot(subset(All_reads_annotated, Stop == FALSE), aes(Score, RNAfold)) + geom_point()
ggplot(subset(All_reads_annotated, Stop == FALSE), aes(RFU, RNAfold)) + geom_point()

write.table(All_reads_annotated, "all_reads_e1_annotated.txt")
write.csv(All_reads_annotated, "all_reads_e1_annotated.csv")
...
ggplot(All_reads, aes(Score)) + 
  ...
  theme_science() + 
  scale_y_continuous(limits=c(0,0.7), expand = c(0,0))
ggsave("density_exp1.tiff", height =3.5, width=3.5, units="in")
...
ggsave("tai_exp1.tiff", height = 3.5, width = 3.5, units = "in")
...
ggplot(All_reads_nostop, aes(as.factor(GC), Score)) + geom_boxplot(notch=TRUE) + theme_science() + xlab("Number of G or C nt") + ylab("GFP Score")
ggsave("GC_exp1.tiff", height =3, width=5, units="in")
...
All_reads_nostop$motif4 <- grepl("CCAGG", All_reads_nostop$Sequence)

m4 <- function(x) { 
  if(!grepl("CCAGG", x)) y <- "Absent"
  if(grepl("CCAGG....", x)) y <- "Position 1"
  if(grepl(".CCAGG...", x)) y <- "Position 2"
  if(grepl("..CCAGG..", x)) y <- "Position 3"
  if(grepl("...CCAGG.", x)) y <- "Position 4"
  if(grepl("....CCAGG", x)) y <- "Position 5"
  return(y)
}
...
All_reads_nostop$motif1_Ile <- gsub("T", "U", All_reads_nostop$motif1_Ile)
...
All_reads_nostop$motif1_v_p <- gsub("T", "U", All_reads_nostop$motif1_v_p)
...
ggplot(subset(All_reads_nostop, !motif1_v_p == "NIF"), aes(x = reorder(motif1_v_p,motif1_v_p, function(x)-length(x)), Score, fill = reorder(motif1_v_p,motif1_v_p, function(x)-length(x)))) + 
  geom_boxplot(notch=TRUE) + scale_fill_manual(values=gray, guide = FALSE) +
  theme_science() + scale_x_discrete(labels = function(x) str_wrap(x, width = 10), limits=c("Absent", "AAGAUU Position 1", "AACAUU Position 1", "AAAAUU Position 1","AAGAUU Position 4", "AACAUU Position 4", "AAAAUU Position 4")) +
  xlab("Motif 1 Variants") + ylab("GFP Score")
...
ggplot(All_reads_nostop, aes(x = reorder(motif2_TAC, motif2_TAC, function(x)-length(x)), Score, fill = reorder(motif2_TAC, motif2_TAC, function(x)-length(x)))) + 
  geom_boxplot(notch=TRUE) + scale_fill_manual(values = gray, guide = FALSE) +
  theme_science() + scale_x_discrete(labels = c("Absent", "AADUAU", "AADUAC")) +
...
All_reads_nostop$motif2_TAC_p <- gsub("T", "U", All_reads_nostop$motif2_TAC_p)

ggplot(subset(All_reads_nostop, !motif2_TAC_p == "NIF"), aes(motif2_TAC_p, Score)) + 
  geom_boxplot(notch=TRUE, fill="#F0E442") + 
  theme_science() + scale_x_discrete(labels = function(x) str_wrap(x, width = 10), limits=c("Absent", "AADUAU Position 1", "AADUAC Position 1", "AADUAU Position 4", "AADUAC Position 4")) +
  xlab("Motif 2 Variants") + ylab("GFP Score")
...
All_reads_nostop$motif2_d_p <- gsub("T", "U", All_reads_nostop$motif2_d_p)
...
ggplot(subset(All_reads_nostop, !motif2_d_p == "NIF"), aes(x = reorder(motif2_d_p,motif1_v_p, function(x)-length(x)), Score, fill = reorder(motif2_d_p,motif1_v_p, function(x)-length(x)))) + 
  geom_boxplot(notch=TRUE) + scale_fill_manual(values = gray, guide = FALSE) +
  theme_science() + scale_x_discrete(labels = function(x) str_wrap(x, width = 10), limits=c("Absent", "AAGUAU Position 1", "AAUUAU Position 1", "AAAUAU Position 1", "AAGUAU Position 4", "AAUUAU Position 4", "AAAUAU Position 4")) +
  xlab("Motif 2 Variants") + ylab("GFP Score")
...
Plasmid_bin1_2_e2  <- merge(Plasmid_bin1_e2, Bin2_e2, by="Sequence", all = TRUE)
Plasmid_bin1_2_3_e2  <- merge(Plasmid_bin1_2_e2, Bin3_e2, by="Sequence", all = TRUE)
Plasmid_bin1_2_3_4_e2 <- merge(Plasmid_bin1_2_3_e2, Bin4_e2, by="Sequence", all = TRUE)
Plasmid_bin1_2_3_4_5_e2 <- merge(Plasmid_bin1_2_3_4_e2, Bin5_e2, by="Sequence", all = TRUE)
...
#normalize each bin by total reads in bin
bin1 <- sum(All_reads_e2$Reads_B1)
bin2 <- sum(All_reads_e2$Reads_B2)
bin3 <- sum(All_reads_e2$Reads_B3)
bin4 <- sum(All_reads_e2$Reads_B4)
bin5 <- sum(All_reads_e2$Reads_B5)

All_reads_e2$Norm_B1 <- All_reads_e2$Reads_B1/bin1
All_reads_e2$Norm_B2 <- All_reads_e2$Reads_B2/bin2
All_reads_e2$Norm_B3 <- All_reads_e2$Reads_B3/bin3
All_reads_e2$Norm_B4 <- All_reads_e2$Reads_B4/bin4
All_reads_e2$Norm_B5 <- All_reads_e2$Reads_B5/bin5


#get sum of all bins normalized
All_reads_e2$total_bins <- All_reads_e2$Norm_B1 + All_reads_e2$Norm_B2 + All_reads_e2$Norm_B3 + All_reads_e2$Norm_B4 + All_reads_e2$Norm_B5
All_reads_e2$total_bins_unnorm <- All_reads_e2$Reads_B1 + All_reads_e2$Reads_B2 + All_reads_e2$Reads_B3 + All_reads_e2$Reads_B4 + All_reads_e2$Reads_B5



write.table(All_reads_e2, "all_reads_e2.txt")

All_reads_e2 <- subset(All_reads_e2, !total_bins_unnorm < 10)

sum(All_reads_e2$total_bins_unnorm)
mean(All_reads_e2$total_bins_unnorm)

ggplot(All_reads_e2, aes(log10(total_bins_unnorm))) + geom_histogram() 
ggsave("hist_total_counts_exp2.tiff")

All_reads_e2$B1_ratio <- All_reads_e2$Norm_B1/All_reads_e2$total_bins
All_reads_e2$B2_ratio <- All_reads_e2$Norm_B2/All_reads_e2$total_bins
All_reads_e2$B3_ratio <- All_reads_e2$Norm_B3/All_reads_e2$total_bins
All_reads_e2$B4_ratio <- All_reads_e2$Norm_B4/All_reads_e2$total_bins
All_reads_e2$B5_ratio <- All_reads_e2$Norm_B5/All_reads_e2$total_bins
...
All_reads_e2_annotated <- merge(All_reads_e2, RNAfold, by = "Sequence", all.x = TRUE)

write.table(All_reads_e2_annotated, "all_reads_e2_annotated.txt")
write.csv(All_reads_e2_annotated, "all_reads_e2_annotated.csv")
...
#write.fasta(low, "low.fasta")
category <- c(rep(1, length(high_e2)), rep(0, length(low_e2))) 
...
ggplot(All_reads_both, aes(Score.x, Score.y)) + geom_point(alpha = 0.05) + theme_science() +
  xlab("GFP Score  - Replicate 1") + ylab("GFP Score  - Replicate 2")
ggsave("replicates.tiff", height = 4, width =  4, units  = "in")

cor.test(All_reads_both$Score.x, All_reads_both$Score.y, method = "pearson")

All_reads_both_100 <- subset(All_reads_both, total_bins_unnorm.x > 100 & total_bins_unnorm.y > 100)

ggplot(All_reads_both_100, aes(Score.x, Score.y)) + geom_point(alpha = 0.05) + theme_science() +
  xlab("GFP Score  - Replicate 1") + ylab("GFP Score  - Replicate 2")
ggsave("replicates_100_reads.tiff", height = 4, width =  4, units  = "in")

cor.test(All_reads_both_100$Score.x, All_reads_both_100$Score.y, method = "pearson")

All_reads_both_200 <- subset(All_reads_both, total_bins_unnorm.x > 200 & total_bins_unnorm.y > 200)

ggplot(All_reads_both_200, aes(Score.x, Score.y)) + geom_point(alpha = 0.05) + theme_science() +
  xlab("GFP Score  - Replicate 1") + ylab("GFP Score  - Replicate 2")
ggsave("replicates_200_reads.tiff", height = 4, width =  4, units  = "in")

cor.test(All_reads_both_200$Score.x, All_reads_both_200$Score.y, method = "pearson")
...
All_reads_no_amber <- subset(All_reads, Amber == "Absent")
...
All_reads_nostop_short <- All_reads_nostop[,names(All_reads_e2_nostop)]

All_reads_nostop_short$DNA <- NULL
All_reads_e2_nostop$DNA <- NULL

All_reads_nostop_both <- rbind(All_reads_nostop_short, All_reads_e2_nostop)

median(All_reads_nostop_both$Score[All_reads_nostop_both$KNIY == TRUE])
sd(All_reads_nostop_both$Score[All_reads_nostop_both$KNIY == TRUE])

median(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 1"])
sd(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 1"])

median(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 2"])
sd(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 2"])

median(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 3"])
sd(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 3"])

median(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 4"])
sd(All_reads_nostop$Score[All_reads_nostop$motif2_all == "Position 4"])

All_reads_nostop_both_merge <- merge(All_reads_nostop, All_reads_e2_nostop, by = "Sequence")

Reporters <- subset(All_reads_nostop_both_merge, Sequence %in% c("AAATTCAGT",
                                                                 "ATCGGTAAG",
                                                                 "ACCGTGGGT",
                                                                 "TTACAGATC",
                                                                 "AATTGTACA",
...
Reporters <- data.frame(Sequence = Reporters$Sequence, AA = Reporters$AA_character.x, Score_Experiment_1 = Reporters$Score.x, Score_Experiment_2 = Reporters$Score.y)

write.csv(Reporters, "Reporters.csv")
